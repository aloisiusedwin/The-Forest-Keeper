...
@export var footstep_sounds_dirt : Array[AudioStreamMP3]
...
func play_sound():
	...
	if scene_name == "Forest":
		...
		var random_index : int = randi_range(0, footstep_sounds_dirt.size() - 1)
		audio_player.stream = footstep_sounds_dirt[random_index]
